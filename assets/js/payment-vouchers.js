...
async function loadVouchers() {
    try {
        const response = await fetch('api/payment-vouchers.php?action=list');
...
async function loadAccounts() {
    try {
        const response = await fetch('api/payment-vouchers.php?action=get_accounts');
        const data = await response.json();
...
async function openVoucherForm() {
    ...
    try {
        const response = await fetch('api/payment-vouchers.php?action=get_next_number');
        const data = await response.json();
...
async function saveVoucher(event) {
    ...
    try {
        ...
        const response = await fetch(`api/payment-vouchers.php?action=${action}`, {
            method: 'POST',
            headers: {
...
async function editVoucher(id) {
    try {
        const response = await fetch(`api/payment-vouchers.php?action=get&id=${id}`);
        const data = await response.json();
        ...
        if (data.success) {
            ...
            document.getElementById('modalTitle').textContent = 'تعديل سند الصرف';
...
async function deleteVoucher(id) {
    ...
    try {
        const response = await fetch('api/payment-vouchers.php?action=delete', {
            method: 'POST',
            headers: {
...
async function postVoucher(id) {
    ...
    try {
        const response = await fetch('api/payment-vouchers.php?action=post', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
...
function printVoucher(id) {
    window.open(`print-payment-voucher.php?id=${id}`, '_blank');
}
...
document.getElementById('searchInput')?.addEventListener('input', function(e) {
    ...
    const filtered = vouchers.filter(v => 
        ...
        v.paidTo.toLowerCase().includes(searchTerm) ||
