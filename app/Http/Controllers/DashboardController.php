...
class DashboardController extends Controller
{
    ...
    private function mainDashboard()
    {
        ...
        $units_stats = Unit::withCount('companies')->get()->map(function($unit) {
            $transfers_count = ClearingTransaction::where('source_company_id', $unit->companies->pluck('id'))
                ->orWhere('target_company_id', $unit->companies->pluck('id'))
                ->count();
    ...
    {
        // واجهة وحدة العمل (سيتم تطويرها لاحقاً)
