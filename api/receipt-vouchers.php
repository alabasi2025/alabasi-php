...
function listVouchers($pdo) {
    $vouchers = $pdo->query("
        ...
        LEFT JOIN accounts da ON rv.debitAccountId = da.id
        LEFT JOIN accounts ca ON rv.creditAccountId = ca.id
        LEFT JOIN users u ON rv.createdBy = u.id
        LEFT JOIN users pu ON rv.postedBy = pu.id
        ORDER BY rv.voucherDate DESC, rv.createdAt DESC
    ")->fetchAll();
...
function createVoucher($pdo, $userId) {
    ...
    // التحقق من البيانات
    if (empty($data['voucherDate']) || empty($data['receivedFrom']) || 
    ...
    try {
        ...
        $voucherNumber = generateVoucherNumber($pdo, 'receipt');
        
        // إدراج السند
        $stmt = $pdo->prepare("
            INSERT INTO receipt_vouchers (
                voucherNumber, voucherDate, receivedFrom, amount, amountInWords,
        ...
        if (($data['status'] ?? 'draft') === 'posted') {
            $journalId = createJournalEntry($pdo, $voucherId, $data, $userId, 'receipt');
            
            // تحديث السند بربطه بالقيد
            $pdo->prepare("UPDATE receipt_vouchers SET journalId = ?, postedAt = NOW(), postedBy = ? WHERE id = ?")
...
function updateVoucher($pdo, $userId) {
    ...
    $stmt = $pdo->prepare("SELECT status FROM receipt_vouchers WHERE id = ?");
    $stmt->execute([$id]);
    $voucher = $stmt->fetch();
...
function deleteVoucher($pdo, $id) {
    // التحقق من وجود السند
    $stmt = $pdo->prepare("SELECT status, journalId FROM receipt_vouchers WHERE id = ?");
    $stmt->execute([$id]);
    $voucher = $stmt->fetch();
    ...
    // حذف السند
    $stmt = $pdo->prepare("DELETE FROM receipt_vouchers WHERE id = ?");
    $stmt->execute([$id]);
...
function postVoucher($pdo, $userId, $id) {
    ...
    try {
        // جلب بيانات السند
        $stmt = $pdo->prepare("SELECT * FROM receipt_vouchers WHERE id = ?");
        $stmt->execute([$id]);
        $voucher = $stmt->fetch();
        ...
        // إنشاء قيد محاسبي
        $journalId = createJournalEntry($pdo, $id, $voucher, $userId, 'receipt');
...
function createJournalEntry($pdo, $voucherId, $data, $userId, $type) {
    // إنشاء قيد محاسبي
    $description = "سند قبض رقم: " . ($data['voucherNumber'] ?? $voucherId);
...
function getNextVoucherNumber($pdo) {
    $number = generateVoucherNumber($pdo, 'receipt');
    
    // إرجاع الرقم للخلف (لم يتم الحفظ بعد)
    $pdo->prepare("UPDATE voucher_sequences SET currentNumber = currentNumber - 1 WHERE voucherType = 'receipt' AND year = ?")
        ->execute([date('Y')]);
